...
Names: Jacob Dupuis & Dalton Lambert
...
```{r}
newLightObv <- lightObv %>%
mutate(vel_air = 12 + velocity,
       vel_vac = 80 + vel_air)
glimpse(newLightObv)
```
...
```{r}
library(moments)
tibble(statistic = c("Mean", "SD", "Skewness", "Kurtosis"),
       data      = c(mean(lightObv$velocity),
                     sd(lightObv$velocity),
                     skewness(lightObv$velocity),
                     kurtosis(lightObv$velocity)))
```

##### Create a histogram of the data. Include an appropriate number of bins.

```{r}
ggplot(lightObv, aes(x = velocity)) +
  geom_histogram(bins = 10, fill = "lightblue", color = "white") +
  labs(title = "Count of Light Velocity",
       subtitle = "The distribution of velocity from the Michelson dataset") +
  theme_minimal()
```
...
ggplot(lightObv, aes(x = velocity)) + 
  ...
  labs(title = "Distribution of Light Velocity",
       subtitle = "Outlier are marked was blue",
       x = "velocity",
       y = "count") + 
  theme_minimal()
```
...
```{r}
ggplot(lightObv, aes(sample = velocity)) +
  stat_qq() +
  stat_qq_line(
    color = "lightblue",
    size = .75
...
```{r}
t_test(newLightObv, response = vel_air) %>% 
...
```{r}
t_test(newLightObv, response = vel_vac) %>% 
  select(lower_ci, upper_ci)
```
...
```{r}
bootstrap_distribution <- newLightObv %>%
...
**ANSWER:** Yes, the necessary assumptions are met for each of the three estimation interval methods because they all output relatively similar results and each one displays a pattern of the velocity of light being higher in a vacuum. For exploration 2 and 3 or method 1 and 2, it gave both an upper and lower confidence interval which is consistent with the other last method for which the output falls between said intervals. 
...
**ANSWER:** No, the currently accepted true values of the velocity of light is calculated in the first table where the mean velocity was 852.4. Looking at the interval estimates, they are all higher if not much higher than the "true" values of velocity. The Graphs all show signs of high amounts of variability and the tables kutosis and skewness values are strong signifiers that this data is also inconsistent. I think because of the original data set, the modified values made by Michelson are also flawed. The logic is there and the effects of Michelson's calculations are present since the estimates were higher but no, none of the estimates contian the "true" intervals.
