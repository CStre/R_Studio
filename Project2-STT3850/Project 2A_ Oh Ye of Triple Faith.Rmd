...
require(broman)
require(BMS)
NAMELET  <- c("C", "o", "l", "l", "i", "n", " ", "S", "t", "r", "e", "i", "t", "m", "a", "n") # change to your name
n        <- length(NAMELET)
...
```{r}
n <- 10^9    # Amount of bits per second being analysed
p <- 0.001   # Probability of a bit being processed incorrectly
```
...
```{r}
randvals <- tibble(x = rbinom(100000, n, p))   # replace the zeroes with appropriate values
```
...
ggplot() +
  ...
  labs(
    title = "informative title here", # add an appropriate title
    x = "description and units of x", # add an appropriate label
    y = "f(x)"
  ) +
  theme_minimal()
```
